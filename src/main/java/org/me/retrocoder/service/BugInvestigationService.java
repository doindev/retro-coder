...
/**
 * Service for investigating bugs using Claude.
 */
@Slf4j
@Service
...
public class BugInvestigationService {
    ...
    private static final String TEMPLATE_PATH = "templates/bug_investigation_prompt.template.md";

    /**
     * Investigate a bug and return suggested fix steps.
     */
    public BugInvestigationResponseDTO investigate(String projectName, BugInvestigationRequestDTO request) {
        ...
        try {
            ...
            prompt = prompt.replace("{{BUG_DESCRIPTION}}", request.getDescription());
            ...
            log.info("Bug investigation completed for project: {}", projectName);
            webSocketSessionManager.broadcastLog(projectName,
                    "\n*** BUG INVESTIGATION COMPLETE ***");

            // Parse the JSON response
            return parseResponse(request.getDescription(), response);

        } catch (Exception e) {
            log.error("Bug investigation failed for project: {}", projectName, e);
            webSocketSessionManager.broadcastLog(projectName,
                    "\n*** BUG INVESTIGATION FAILED: " + e.getMessage() + " ***");
            return BugInvestigationResponseDTO.failure(request.getDescription(), e.getMessage());
        } finally {
            if (client != null) {
                client.close();
            }
    ...
    /**
     * Load the bug investigation prompt template.
     */
     ...
     */
    private BugInvestigationResponseDTO parseResponse(String description, String response) {
        try {
            // Try to extract JSON from the response
            String json = extractJson(response);
            if (json == null) {
                log.warn("No JSON found in response, using fallback parsing");
                return createFallbackResponse(description, response);
            }

            JsonNode root = objectMapper.readTree(json);

            String rootCause = getTextOrDefault(root, "rootCause", "Unable to determine root cause");
            List<String> affectedFiles = getStringList(root, "affectedFiles");
            List<String> steps = getStringList(root, "steps");
            String additionalNotes = getTextOrDefault(root, "additionalNotes", "");

            // Generate a suggested name from the root cause
            String suggestedName = generateSuggestedName(rootCause, description);

            return BugInvestigationResponseDTO.success(
                    description,
                    rootCause,
                    affectedFiles,
                    steps,
     ...
     */
    private BugInvestigationResponseDTO createFallbackResponse(String description, String response) {
        List<String> steps = new ArrayList<>();
        steps.add("Review the investigation output above");
        steps.add("Identify the files mentioned in the analysis");
        steps.add("Apply the suggested fixes");
        steps.add("Test to verify the bug is resolved");

        return BugInvestigationResponseDTO.success(
                description,
                "See investigation output for details",
                List.of(),
                steps,
                response.length() > 500 ? response.substring(0, 500) + "..." : response,
                generateSuggestedName("", description)
        );
    }
    ...
    /**
     * Generate a suggested feature name from the root cause.
     */
    private String generateSuggestedName(String rootCause, String description) {
        String source = rootCause.isEmpty() ? description : rootCause;
        // Take first sentence or first 50 chars
        String name = source.split("[.!?]")[0].trim();
        if (name.length() > 60) {
            name = name.substring(0, 57) + "...";
        }
        if (!name.toLowerCase().startsWith("fix")) {
            name = "Fix: " + name;
        }
        return name;
    }
